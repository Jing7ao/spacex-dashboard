...
BATCH10 = [
    ('300476', '胜宏科技', '胜宏科技：2025年年度报告 (1).pdf'),
    ('688017', '绿的谐波', '绿的谐波：2025年年度报告 (1).pdf'),
    ('300070', '碧水源', '碧水源：2025年年度报告.pdf'),
    ('601877', '正泰电器', '正泰电器：正泰电器2025年年度报告.pdf'),
    ('002340', '格林美', '格林美：2025年年度报告.pdf'),
    ('600362', '江西铜业', '江西铜业：江西铜业股份有限公司2025年年度报告.pdf'),
    ('300249', '依米康', '依米康：2025年年度报告.pdf'),
    ('600126', '杭钢股份', '杭钢股份：杭州钢铁股份有限公司2025年年度报告.pdf'),
    ('600008', '首创环保', '首创环保：首创环保2025年年度报告.pdf'),
    ('601789', '宁波建工', '宁波建工：宁波建工2025年年度报告.pdf'),
]
...
DOMESTIC = re.compile(r'华为|字节|阿里|腾讯|百度|昇腾|寒武纪|GPU|PCB')
...
def main():
    print('='*70)
    print('BATCH 10 ANNUAL REPORT AUDIT')
    print('='*70)

    all_findings = {}

    for code, name, filename in BATCH10:
        pdf_path = os.path.join(DIR, filename)
        if not os.path.exists(pdf_path):
            # fallback to non-(1) version
            fallback = filename.replace(' (1).pdf', '.pdf')
            pdf_path = os.path.join(DIR, fallback)
        if not os.path.exists(pdf_path):
        ...
        for kw in ['华为', '字节', '阿里', '腾讯', '百度', 'GPU', 'PCB']:
            for pos, ctx in find_context(text, kw, 80):
